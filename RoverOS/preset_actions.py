...
"""
These are the raw actions to control the roverbyte's servos.
New actions can be defined here, and referenced in action_flow.py
"""
...
def pray(my_dog):
    ...
    sit_position = [30, 60, -30, -60, 80, -45, -80, 45]

    # Raise both front legs
    raise_legs = [
        [30, 60, 70, 70, 80, -45, -80, 45],
    ]

    # Bring paws together
    paws_together = [
        [30, 60, 70, 70, 80, -45, -80, 45],
        [30, 60, 60, 60, 80, -45, -80, 45],
        [30, 60, 50, 50, 80, -45, -80, 45],
    ]

    # Lower legs back to sitting position
    lower_legs = [
        [30, 60, 30, 30, 80, -45, -80, 45],
        [30, 60, 0, 0, 80, -45, -80, 45],
        [30, 60, -30, -30, 80, -45, -80, 45],
    ]

    # Execute the praying motion
    my_dog.legs_move([sit_position], immediately=False, speed=80)
    my_dog.wait_all_done()
    sleep(0.5)

    my_dog.legs_move(raise_legs, immediately=False, speed=80)
    my_dog.wait_all_done()
    sleep(0.5)

    for _ in range(2):  # Repeat the paws together motion twice
        my_dog.legs_move(paws_together, immediately=False, speed=60)
        my_dog.wait_all_done()
        sleep(0.3)

    my_dog.legs_move(lower_legs, immediately=False, speed=60)
    my_dog.wait_all_done()
...
def butt_up(my_dog):
    ...
    # Raise rear end
    butt_up_position = [30, 60, -30, -60, 40, 0, -40, 0]

    # Execute the butt up motion
    my_dog.legs_move([sit_position], immediately=False, speed=80)
    my_dog.wait_all_done()
    sleep(0.5)

    my_dog.legs_move([butt_up_position], immediately=False, speed=60)
    my_dog.wait_all_done()
    
    return butt_up_position  # Return the butt up position for use in other functions

def twerk(my_dog, cycles=3):
    # Get the butt up position
    butt_up_position = butt_up(my_dog)
    
    # Twerk positions
    twerk_up = [30, 60, -30, -60, 30, 0, -30, 0]
    twerk_down = [30, 60, -30, -60, 50, 0, -50, 0]

    # Execute the twerk motion
    for _ in range(cycles):
        my_dog.legs_move([twerk_up], immediately=False, speed=100)
        my_dog.wait_all_done()
        my_dog.legs_move([twerk_down], immediately=False, speed=100)
        my_dog.wait_all_done()

    # Return to butt up position
    my_dog.legs_move([butt_up_position], immediately=False, speed=80)
    my_dog.wait_all_done()

    # Return to sitting position
    sit_position = [30, 60, -30, -60, 80, -45, -80, 45]
    my_dog.legs_move([sit_position], immediately=False, speed=60)
    my_dog.head_move([[0, 0, -35]], speed=80)
    my_dog.wait_all_done()

def dab(my_dog):
    # Dab position
    dab_position = [30, 60, 70, -60, 80, -45, -80, 45]  # Raise one front leg
    head_position = [-30, 0, -45]  # Turn head to the side and down

    my_dog.legs_move([dab_position], immediately=False, speed=80)
    my_dog.head_move([head_position], immediately=False, speed=80)
    my_dog.wait_all_done()
    sleep(1)
...
def floss(my_dog, cycles=3):
    left_position = [40, 60, -40, -60, 70, -45, -70, 45]
    right_position = [20, 60, -20, -60, 90, -45, -90, 45]

    for _ in range(cycles):
        my_dog.legs_move([left_position], immediately=False, speed=100)
        my_dog.wait_all_done()
        my_dog.legs_move([right_position], immediately=False, speed=100)
        my_dog.wait_all_done()
...
def woah(my_dog):
    # Woah position
    woah_position = [30, 60, 50, 50, 60, -20, -60, 20]  # Raise both front legs
    head_position = [0, 0, -60]  # Look up

    my_dog.legs_move([woah_position], immediately=False, speed=70)
    my_dog.head_move([head_position], immediately=False, speed=70)
    my_dog.wait_all_done()
    sleep(1)
...
def gangnam_style(my_dog, cycles=4):
    left_position = [30, 60, 50, -60, 80, -45, -80, 45]  # Left paw up
    right_position = [30, 60, -30, 50, 80, -45, -80, 45]  # Right paw up

    for _ in range(cycles):
        my_dog.legs_move([left_position], immediately=False, speed=90)
        my_dog.wait_all_done()
        my_dog.legs_move([right_position], immediately=False, speed=90)
        my_dog.wait_all_done()
...
def bottle_flip(my_dog):
    # Prepare to "flip"
    prepare_position = [30, 60, 50, 50, 60, -20, -60, 20]  # Raise front legs
    my_dog.legs_move([prepare_position], immediately=False, speed=80)
    my_dog.wait_all_done()

    # Quick "flip" motion
    flip_position = [30, 60, -50, -50, 100, -70, -100, 70]
    my_dog.legs_move([flip_position], immediately=False, speed=100)
    my_dog.wait_all_done()
