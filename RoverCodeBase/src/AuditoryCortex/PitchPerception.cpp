...
namespace AuditoryCortex
{
    ...
    NoteInfo PitchPerception::getNoteInfo(uint16_t frequency) {
        NoteInfo info(NoteIndex::REST, 4, NoteType::QUARTER);
        ...
        for(int i = 0; i < sizeof(NOTE_FREQUENCIES)/sizeof(NOTE_FREQUENCIES[0]); i++) {
            ...
            if(diff < minDiff) {
                minDiff = diff;
                info.note = static_cast<NoteIndex>(i % 12);
                info.octave = (i / 12) + 1;
                info.isSharp = (static_cast<int>(info.note) == 1) || (static_cast<int>(info.note) == 3) || 
                            (static_cast<int>(info.note) == 6) || (static_cast<int>(info.note) == 8) || 
    ...
    uint16_t PitchPerception::getNoteFrequency(const NoteInfo& info) {
        return NOTE_FREQUENCIES[(info.octave - 1) * 12 + static_cast<int>(info.note)];
    }
    ...
    uint16_t PitchPerception::getNoteDuration(NoteType note, TimeSignature timeSignature) {
        /*
        timeSignature: Represents the denominator of the time signature.
        For example:
            - TIME_4_4 (4) = Quarter note gets the beat
            - TIME_6_8 (8) = Eighth note gets the beat
            - TIME_2_2 (2) = Half note gets the beat
        */
        ...
        switch (timeSignature) {
            case TimeSignature::TIME_2_2: baseDuration = WHOLE_NOTE_MS / 2; break;  // Half note gets the beat
            case TimeSignature::TIME_4_4: baseDuration = WHOLE_NOTE_MS / 4; break;  // Quarter note gets the beat
            case TimeSignature::TIME_6_8: baseDuration = WHOLE_NOTE_MS / 8; break;  // Eighth note gets the beat
            case TimeSignature::TIME_12_16: baseDuration = WHOLE_NOTE_MS / 16; break; // Sixteenth note gets the beat
            default: baseDuration = WHOLE_NOTE_MS / 4; break;  // Default to quarter note
        }
