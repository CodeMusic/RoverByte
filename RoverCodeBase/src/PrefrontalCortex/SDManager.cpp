...
void SDManager::init(){
    // SPI and CS pins are already initialized by SPIManager
    // Just verify CS pins are in correct state before mounting
    SPIManager::deselectAll();
    ...
    LOG_DEBUG("SD Card Type: %s", cardTypeStr);

    uint64_t cardSize = SD.cardSize() / (1024 * 1024);
    uint64_t totalSpace = SD.totalBytes() / (1024 * 1024);
    uint64_t usedSpace = SD.usedBytes() / (1024 * 1024);

    LOG_DEBUG("SD Card Size: %llu MB", cardSize);
    LOG_DEBUG("Total space: %llu MB", totalSpace);
    LOG_DEBUG("Used space: %llu MB", usedSpace);
    
    SDManager::initialized = true;
}
